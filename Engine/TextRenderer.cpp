...
void McCol::TextRenderer::CreateTextLayout(D2DRender* renderer)
{
	...
	// 폰트 이름(설치된 폰트 사용)
	m_pDWriteTextLayout->SetFontFamilyName(m_FontName.c_str(), { m_StartRange, m_EndRange });		                
	// 폰트 두께(Bold, Light)
	m_pDWriteTextLayout->SetFontWeight(m_FontWeights[m_FontWeightIndex], { m_StartRange, m_EndRange });          
	// 폰트 스타일(기울기)
	m_pDWriteTextLayout->SetFontStyle(m_FontStyles[m_FontStyleIndex], { m_StartRange, m_EndRange });             
	// 폰트 크기
	m_pDWriteTextLayout->SetFontSize(m_FontSize, { m_StartRange, m_EndRange });			                        
	// 특정 구간 폰트 색상 설정
	//m_pDWriteTextLayout->SetDrawingEffect(renderer->GetWhiteBrush(), { m_StartRange, m_EndRange });	            
